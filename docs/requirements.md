...
用户要求先补齐功能再集中实机检查，不再逐项要求截图或重开；当前运行中的分身、微信与 GUI 保持。授权继续本地开发，新增仅限 loopback、每轮随机密钥的分身管理入口，使未来新版 GUI 启动后可以查询状态、重连、启动不发送接收和停止工作进程。不得以此增加群发送授权、任意命令执行、系统设置修改或凭据保存。全部缺口补齐后再邀请用户进行其同意的最后一次新版启动，不能现在要求操作。

## 待用户主动启用的本轮收发模式

实现GUI收发授权选项不等于已获得授权。默认仍不发送；用户必须勾选本轮文字/图片发送，并点击收发启动、核对明确群列表后确认。授权不落配置文件，只随本次工作进程传递，停止、失败、重启后失效。程序仅能在通过隔离检查的分身及本轮配置群发送；agent本轮不启用该选项、不发送真实消息。

## 2026-09-15 最新输入授权（覆盖下方历史全输入禁止）

用户明确回复“允许，不影响我主windows使用电脑即可”，允许分身内机器人点击、按键、焦点及剪贴板操作。主Windows不得被操作；错误、过期连接、锁屏、窗口重启或多主窗口时停止，不回退主机。分身内唯一微信就是用户登录的小号，不再增加大小号选择。

这是分身输入授权，不是群消息发送授权。当前新增ChildWxAutoAdapter仍拒绝文字/图片发送；未完成真实客户端兼容性、隐藏后操作及主机输入互不干扰验收，不能将新增代码称为生产可用。旧主机后台启动禁用保持不变。
