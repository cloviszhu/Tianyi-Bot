...
`dist/trial-0.7.0` 包含本地构建 wheel、摘要清单和 `launch_child.pyw`。使用已有 Python 环境，不覆盖正在运行的安装，不重启分身或微信。`src/install_trial_shortcut.ps1` 仅创建“天意Bot试运行版”桌面入口，直接指向 pythonw 与启动文件，不经命令脚本链，不修改安全软件设置；原有快捷方式保留。

入口校验相邻 wheel 后加载；仅对自身及其子进程设置 PYTHONPATH，使窗口扫描、监听、视觉识别辅助进程与界面同版本。独立后台版本选择使用隔离加载并校验摘要，允许下一次启动切换和回退，不改变正在运行的进程。分身检查和本轮发送确认仍保留。

试运行集中检查：启动/停止、加图及原图保存、重复消息、关键词取图、断连、隐藏分身后的持续运行和主机输入不受干扰。文字/图片收发入口已经实现，但真实微信版本兼容性、连续消息发送者缺失和界面消息 ID 稳定性仍是试运行待查项；不把本地回归写成这些项目已经通过。
...
失败报告另保留有界的项目/免费库/UIA包模块名和代码行号，用于追踪普通 Exception 的来源；不保留完整路径、源码行、局部变量或异常正文。旧报告不含这些位置，不能逆推补全。

0.6.14 修复绑定与输入检查使用不同类名来源的问题：两者均按微信进程 + UIA ClassName/ProcessId 判断主窗口，不把 UIA 的 `mmui::MainWindow` 与原生 Win32 Qt 类名混用。会话、进程创建时间、路径、唯一窗口、最小化与连接租约检查保留。隔离失败报告新增固定细分步骤。此修复由本地模拟回归验证，真实失败是否仅由此造成仍需新版报告确认。

0.6.13 补上启动阶段失败报告，固定位置为用户目录 `.tianyi-bot/intake-failure.json`，仅含步骤代码、白名单异常类型和时间，不含异常正文、截图或账号。报告是最近一次失败，不是当前在线状态。旧版已吞掉的异常无法追溯，需新版再次运行取得证据。

用户确认后，`startup-repair` 变更将 `.codex-remote-attachments/*` 分类为审计附件，Git 仍忽略此目录，不上传照片。原 `gallery-mvp` 历史与未完成验收保留；新变更不代表全量上线完成。
...
以完整版本一次准备，开发回归由维护者运行；用户不再逐阶段截图、重开。真实试运行仍须在指定测试群中由用户于 GUI 确认本轮发送。主 Windows 输入隔离、群范围和图库完整性保留。

后台代码支持并存 wheel：`python -m wechat_gallery_bot.management.worker_release stage <本地项目wheel>` 返回内容摘要；`activate <摘要>` 选择下一次工作进程加载的包；`rollback` 切回上一个选择。均不启动机器人、不重启 GUI/分身/微信，不修改第三方依赖。包错误不静默回退。SHA-256 检查完整性，不证明发布者身份，只使用本项目已审查的本地构建产物。

可移植 GUI 包由 `launch_child.pyw`、`release.json`、对应 wheel 组成；使用现有 host-runtime 的 pythonw.exe 启动该脚本。脚本校验同目录固定项目包后加载 GUI，新 GUI 与旧安装目录并存。首次启动需要用户在现有分身里关闭旧 GUI 后打开一次新版；此后 worker 可通过本机控制通道停止、选择版本和启动不发送接收，不再需要重开 GUI。GUI 自身更新仍不是热更新，不能把 worker 更新描述为整套 GUI 热替换。
